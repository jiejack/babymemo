    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          resolve(testUsername); // 返回注册的用户名用于登录测试
        } catch (e) {
...
async function runAllTests() {
  ...
  try {
    ...
    // 测试2: API注册
    const username = await testApiRegister();
    console.log('\n--- 测试2 完成 ---, 注册的用户: ' + username + '\n');
    
    // 测试3: API登录
    const token = await testApiLogin(username);
    console.log('\n--- 测试3 完成 ---, 获取到token\n');
    ...
    console.log('🎉 所有测试完成!');
    console.log('\n=== 测试结果总结 ===');
    console.log('✅ 直接数据库连接测试: 成功');
    console.log('✅ API注册测试: 成功');
    console.log('✅ API登录测试: 成功');
    console.log('✅ API获取用户信息测试: 成功');
    process.exit(0);
  } catch (error) {
