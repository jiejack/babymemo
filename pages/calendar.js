...
export default function Calendar() {
  ...
  const handleSaveEvent = (newEvent) => {
    setEvents(prev => {
      const existingEventIndex = prev.findIndex(event => event.id === newEvent.id);
        ...
        <div className="grid grid-cols-7">
          {generateCalendar().map((date, index) => {
            ...
            return (
              <div 
                key={index} 
                className={`min-h-[120px] p-2 border ${isCurrentMonth ? 'bg-white' : 'bg-gray-50'} ${isToday ? 'ring-2 ring-blue-500' : ''} hover:bg-gray-50 cursor-pointer`}
